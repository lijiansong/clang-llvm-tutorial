...
class DataflowVisitor {
 public:
  ...
  virtual void compDFVal(BasicBlock *block, T *dfval,
                         typename DataflowResult<T>::Type *result,
                         map<BasicBlock *, BasicBlock *> &back_edge,
                         set<BasicBlock *> &loop_list, bool isforward) {
    if (isforward == true) {
  ...
  virtual void handlePredIcmp(BasicBlock *pred_bb,
                              typename DataflowResult<T>::Type *result) = 0;
};
