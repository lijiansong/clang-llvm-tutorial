...
typedef pair<map<string, vector<int>>, map<string, vector<int>>>
    true_false_out_pair;

struct LivenessInfo {
  std::set<Instruction *> LiveVars;  /// Set of variables which are live

  // interval of each vars,name of each vars is the key, and vector<int> is its
  // range
  ...
  LivenessInfo()
      : LiveVars(),
        VarRanges(),
        isReachable(true) /*,TrueOut(),FalseOut() */
  {}
...
class LivenessVisitor : public DataflowVisitor<struct LivenessInfo> {
 public:
  ...
  void merge(LivenessInfo *dest, const LivenessInfo &src,
    ...
    for (; it != ie; ++it) {
      // this means the basic block already get the condition range from its
      // pred block
      if ((*result)[current_bb].first.VarRanges.count((*it).first) > 0) {
        ...
        if (dest->VarRanges.count((*it).first) > 0) {
          ...
          dest->VarRanges.insert(
              pair<string, vector<int>>((*it).first, (*it).second));
        }
      }
    }
  }
    ...
    for (; ii != ie; ++ii) {
      ...
      if (isa<BranchInst>(inst)) {
        ...
        if (br_inst->isConditional()) {
          ...
          int predicate = cmpInst->isSigned() ? cmpInst->getSignedPredicate()
                                              : cmpInst->getUnsignedPredicate();
          ...
          if ((lhsConstant &&
               rhsConstant) /*|| (lhsRange.empty() && rhsRange.empty()) */) {
          ...
          // else
          // {
          //     //both have ranges, we have to assign new may ranges to both
          //     variables! if (!lhsRange.empty() && !rhsRange.empty())
          //     {
          //         variable = NULL;
          //     }
          ...
          if (variable && constant) {
            vector<int> variableRange =
                tryGetRange(variable, &((*result)[pred_bb].second));
            // X == C
            if (predicate == CmpInst::ICMP_EQ) {
              vector<int> constantRange;
              map<string, vector<int>> *trueMap =
                  &(((*result)[pred_bb].second).VarRanges);
              if (trueMap->count(variable->getName().str()) > 0) {
                ...
                vector<int> intersection =
                    intersect(variableRange, constantRange);
                if (!intersection.empty()) {
                  //(*result)[trueBlock].first.VarRanges[variable->getName().str()]=intersection;
                  (*result)[trueBlock].first.VarRanges.insert(
                      pair<string, vector<int>>(variable->getName().str(),
                                                intersection));
                } else {
                  // unreachable
                ...
                if (!_cut.empty()) {
                  (*result)[falseBlock].first.VarRanges.insert(
                      pair<string, vector<int>>(variable->getName().str(),
                                                _cut));
                } else {
                  (*result)[falseBlock].first.VarRanges.insert(
            ...
            // X != C
              ...
              vector<int> constantRange;
              // map < string, vector < int >>*falseMap =
              // &(((*result)[pred_bb].second).VarRanges);  falseMap->erase
              ...
              if (!_cut.empty()) {
                (*result)[trueBlock].first.VarRanges.insert(
                    pair<string, vector<int>>(variable->getName().str(), _cut));
              } else {
                (*result)[trueBlock].first.VarRanges.insert(
                    pair<string, vector<int>>(variable->getName().str(),
                                              variableRange));
            ...
            // X > C
              ...
              constantRangeTrue.push_back(
                  INF /*APInt::getSignedMaxValue(32).getSExtValue() */);  // max

              // falseMap.erase(variable->getName().str());
              constantRangeFalse.push_back(
                  MINUS_INF /*APInt::getSignedMinValue(32).getSExtValue() */);  // min
              constantRangeFalse.push_back(constant->getSExtValue());  // max

              vector<int> intersectTrue =
                  intersect(constantRangeTrue, variableRange);
              vector<int> intersectFalse =
                  intersect(constantRangeFalse, variableRange);
            ...
            // X >= C
              ...
              constantRangeFalse.push_back(
                  MINUS_INF /*APInt::getSignedMinValue(32).getSExtValue() */);  // min
              constantRangeFalse.push_back(constant->getSExtValue() -
                                           1);  // max
          ...
          // both operands are ranges
            ...
            vector<int> lhsRange =
                tryGetRange(lhs, &((*result)[pred_bb].second));
            vector<int> rhsRange =
                tryGetRange(rhs, &((*result)[pred_bb].second));
            // assign LHS first
            range = rhsRange;
            variable = lhs;
            vector<int> variableRange =
            ...
            // X < Y,
              ...
              constantRangeTrue.push_back(
                  MINUS_INF /*APInt::getSignedMinValue(32).getSExtValue() */);
              constantRangeTrue.push_back(range[1] - 1);

              constantRangeFalse.push_back(range[0]);
              constantRangeFalse.push_back(
                  INF /*APInt::getSignedMaxValue(32).getSExtValue() */);
      ...
      switch (inst->getOpcode()) {
        ...
        case Instruction::Mul: {
          ...
          if (strcmp(left.c_str(), right.c_str()) == 0) {
            if ((operandRange1[0] <= 0) && (operandRange1[1] <= 0) &&
                (operandRange2[0] <= 0) && (operandRange2[1] <= 0)) {
              resultMin = operandRange1[1] * operandRange2[1];
              resultMax = operandRange1[0] * operandRange2[0];
            } else if (operandRange1[0] <= 0 && operandRange1[1] >= 0 &&
                       operandRange2[0] <= 0 && operandRange2[1] >= 0) {
              resultMin = 0;
              resultMax = max(operandRange1[1] * operandRange2[1],
                              operandRange1[0] * operandRange2[0]);
            } else if (operandRange1[0] >= 0 && operandRange1[1] >= 0 &&
                       operandRange2[0] >= 0 && operandRange2[1] >= 0) {
              resultMin = operandRange1[0] * operandRange2[0];
              resultMax = operandRange1[1] * operandRange2[1];
  ...
  vector<int> processLoop(map<BasicBlock *, BasicBlock *> &back_edge,
      ...
      for (; inst_it != inst_ie; ++inst_it) {
        ...
        if (isa<BranchInst>(inst)) {
          ...
          if (br_inst->isConditional()) {
            Value *condition = br_inst->getCondition();
            ICmpInst *cmpInst = dyn_cast<ICmpInst>(condition);
            int predicate = cmpInst->isSigned()
                                ? cmpInst->getSignedPredicate()
                                : cmpInst->getUnsignedPredicate();
            Value *lhs = cmpInst->getOperand(0);
            Value *rhs = cmpInst->getOperand(1);
            ConstantInt *rhsConstant = tryGetConstantValue(rhs);
        ...
        if (inst->isBinaryOp()) {
          ConstantInt *operandConstant1 =
              tryGetConstantValue(inst->getOperand(0));
          ConstantInt *operandConstant2 =
              tryGetConstantValue(inst->getOperand(1));
          if ((operandConstant1 == NULL) && (operandConstant2 != NULL)) {
            int operand2 = operandConstant2->getSExtValue();
            switch (inst->getOpcode()) {
              case Instruction::Add: {
  ...
  void compDFVal(Instruction *inst, LivenessInfo *dfval,
                 ...
                 set<BasicBlock *> &loop_list) override {
    ...
    if (isa<ReturnInst>(inst)) {
      ...
      if (back_edge.size() > 0) {
        ...
        if (dfval->VarRanges.count(ret_value->getName().str()) > 0) {
          ...
          /*errs ()*/ cout << "Error: " << ret_value->getName().str()
                           << " doesn't exist!\n";
          /*errs ()*/ cout << "************[min..max]\n";
        }
      }
    ...
    if (isa<PHINode>(inst)) {
      ...
      for (unsigned i = 0; i < num; ++i) {
        ...
        if (constant) {
          ...
          map<string, vector<int>> pred_map =
              (*result)[pred_bb].second.VarRanges;
      ...
      //  errs()<<tmp[i]<<'\n';
      // }
      dfval->VarRanges.insert(
          pair<string, vector<int>>(phi_node->getName().str(), tmp));
...
class Liveness : public FunctionPass {
 public:
  ...
  bool runOnFunction(Function &F) override {
    ...
    compForwardDataflow(&F, &visitor, &result, back_edge, loopInfo, loop_list,
                        initval);
