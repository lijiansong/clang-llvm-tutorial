...
vector<BasicBlock *> getLoopHeaderList(LoopInfo *loopInfo,
...
// void getBackEdge(vector<BasicBlock *> worklist,BasicBlock *loopheader,
...
void compForwardDataflow(Function *fn, DataflowVisitor<T> *visitor,
                         ...
                         map<BasicBlock *, BasicBlock *> &back_edge,
                         LoopInfo *loopInfo, set<BasicBlock *> &loop_list,
                         const T &initval) {
    ...
    for (size_t i = 0; i < num; ++i) {
      ...
      if (header != worklist.end()) {
        for (; header != worklist.end(); ++header) {
          ...
          visited[*header] = true;
          for (succ_iterator si = succ_begin(*header), se = succ_end(*header);
               si != se; ++si) {
            // BasicBlock *succ = *si;
            if (!tmp[*si]) loop_list.insert(*si);
          }
        }
      }
    }
  ...
  visitor->compDFVal(bb, &((*result)[bb].second), result, back_edge, loop_list,
                     true);
...
void compBackwardDataflow(Function *fn, DataflowVisitor<T> *visitor,
                          typename DataflowResult<T>::Type *result,
                          map<BasicBlock *, BasicBlock *> &back_edge,
                          set<BasicBlock *> &loop_list, const T &initval) {
